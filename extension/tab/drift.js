 ...
 * (Report.Schema == SCHEMA_SUPPORTED here).
 */
(function () {
  "use strict";

  var ATTACHMENT_TYPE = "tf-snag.report";
  var SCHEMA_SUPPORTED = 1;
  ...
  function renderError(msg) {
    ...
    r.appendChild(el("div", { class: "tfd-banner tfd-banner-error" }, [
      el("strong", { text: "Could not load the drift report." }),
  ...
  function render(result, build) {
    ...
    if (!result.report) {
      r.appendChild(el("div", { class: "tfd-banner" }, [
        el("strong", { text: "No drift report for this run." }),
        el("div", {
          class: "tfd-muted",
    ...
    if (typeof report.schema === "number" && report.schema > SCHEMA_SUPPORTED) {
      r.appendChild(el("div", { class: "tfd-banner tfd-banner-warn" }, [
        ...
        el("div", { class: "tfd-muted", text: "Update the tf-snag-tab extension. Showing a best effort below." })
      ]));
    }

    // status banner
    var hasDrift = drift.length > 0;
    r.appendChild(el("div", { class: "tfd-banner " + (hasDrift ? "tfd-banner-warn" : "tfd-banner-ok") }, [
      el("strong", {
        text: hasDrift
          ? drift.length + " resource" + (drift.length === 1 ? "" : "s") + " changed outside Terraform"
          : "No drift detected"
      }),
      el("div", { class: "tfd-muted", text: metaLine(report, pending, build) })
    ]));

    r.appendChild(section("Changed outside Terraform", drift, true,
      "Nothing has changed outside Terraform for this run."));

    r.appendChild(section("Pending changes from configuration", pending, false,
      "No pending changes — configuration matches state."));

    resize();
  }

  function metaLine(report, pending, build) {
    var bits = [];
  ...
  function section(title, rows, expandFirst, emptyText) {
    ...
    var table = el("table", { class: "tfd-table" }, [
