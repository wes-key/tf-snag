...
`npm run logo` alongside the Marketplace tile: the drift check is the extension
logo redrawn heavier for 32x32 — a magnifying glass with an amber lens showing
the `~` a plan marks a changed resource with — so a step in the run reads as the same thing as
the tab, and the installer
is a download arrow into a tray.
